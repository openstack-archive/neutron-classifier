...
from neutron_classifier.common import utils
...
def _validate_fwr_protocol_parameters(fwr, protocol):
    ...
    if protocol not in (const.PROTO_NAME_TCP, const.PROTO_NAME_UDP):
        source_port_range_min = utils.get_attr_value(
            fwr, 'source_port_range_min')
        source_port_range_max = utils.get_attr_value(
            fwr, 'source_port_range_max')
        destination_port_range_min = utils.get_attr_value(
            fwr, 'destination_port_range_min')
        destination_port_range_max = utils.get_attr_value(
...
def _validate_sg_ethertype_and_protocol(rule, protocol):
    ...
    eth_value = utils.get_attr_value(rule, 'ethertype')
...
def _validate_port_range(min_port, max_port):
    ...
    if int(min_port) > int(max_port):
        ...
        raise exc.InvalidPortValue(port=port_range)
...
def is_ethernetclassifier_valid(rule, type):
    ...
    eth_value = utils.get_attr_value(rule, attr_type)
    if not eth_value:
        return False
...
def is_protocolclassifier_valid(rule, type):
    ...
    protocol = utils.get_attr_value(rule, 'protocol')
...
def is_ipclassifier_valid(rule, type):
    ...
    if type == SG_RULE_TYPE:
        dst_ip_address = utils.get_attr_value(rule, 'remote_ip_prefix')
        attr_type = 'ethertype'
    else:
        src_ip_address = utils.get_attr_value(rule, 'source_ip_address')
        dst_ip_address = utils.get_attr_value(rule, 'destination_ip_address')
        attr_type = 'ip_version'
    ...
    rule_ip_version = utils.get_attr_value(rule, attr_type)
    if type == SG_RULE_TYPE:
...
def is_directionclassifier_valid(rule, type):
    ...
    direction = utils.get_attr_value(rule, 'direction')
    if not direction:
        return False
    return True
...
def is_transportclassifier_valid(rule, type):
    ...
    if type == SG_RULE_TYPE:
        port_range_min = utils.get_attr_value(rule, 'port_range_min')
        port_range_max = utils.get_attr_value(rule, 'port_range_max')
