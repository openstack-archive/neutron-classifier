...
PREFIX = "/ccf"
object_path = PREFIX + "/udp_classifications"
resource = 'udp_classification'
...
class ListUDPClassification(command.Lister):
    ...
    def take_action(self, parsed_args):
        data = self.app.client_manager.neutronclient.list(
            collection='udp_classifications',
        ...
        columns = ('id', 'name', 'description', 'negated', 'shared',
                   'src-port', 'dst-port', ' length', 'window-size')

        return (headers, (utils.get_dict_properties(
            s, columns) for s in data['udp_classifications']))
