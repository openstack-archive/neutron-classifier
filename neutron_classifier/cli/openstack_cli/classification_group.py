...
PREFIX = "/ccf"
object_path = PREFIX + "/classification_groups"
...
class CreateClassificationGroup(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=('DSCP Classification value,'))
        parser.add_argument(
            '--classification-group', nargs='*',
            help=('ECN Classification value.'))
        parser.add_argument(
            '--operator',
            help=('Protocol Classification value.'))
        parser.add_argument(
            '--shared',
            help=('Protocol Classification value.'))
...
def _get_attrs(client_manager, parsed_args, is_create=False):
    ...
    if is_create:
        ...
        if getattr(parsed_args, attribute.replace('-', '_')) is not None:
            attrs['classification_group'] = getattr(
