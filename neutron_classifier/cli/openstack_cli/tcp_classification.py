...
PREFIX = "/ccf"
object_path = PREFIX + "/tcp_classifications"
resource = 'tcp_classification'
...
def _get_attrs(client_manager, parsed_args, is_create=False):
    ...
    if is_create:
        ...
        if getattr(parsed_args, attribute.replace('-', '_')) is not None:
            attrs['src-port'] = getattr(
                parsed_args, attribute.replace('-', '_'))
        attribute = 'dst-port'
        if getattr(parsed_args, attribute.replace('-', '_')) is not None:
            attrs['dst-port'] = getattr(
                parsed_args, attribute.replace('-', '_'))
        if parsed_args.flags is not None:
            attrs['flags'] = parsed_args.flags
        if parsed_args.window is not None:
            attrs['window'] = parsed_args.window
        attribute = 'data-offset'
        if getattr(parsed_args, attribute.replace('-', '_')) is not None:
            attrs['data-offset'] = getattr(
                parsed_args, attribute.replace('-', '_'))
        attribute = 'option-kind'
        if getattr(parsed_args, attribute.replace('-', '_')) is not None:
            attrs['option-kind'] = getattr(
                parsed_args, attribute.replace('-', '_'))
