...
class NeutronClassifierExtension(l2_extension.L2AgentExtension):
    ...
    SUPPORTED_RESOURCE_TYPES = [
        class_group.ClassificationBase.obj_name(),
        class_group.ClassificationGroup.obj_name()]

    def __init__(self):
        print("Sara, init Neutron Classifier L2-extension")
    ...
    def initialize(self, connection, driver_type):
        print("Sara, initializing Neutron Classifier L2-extension")
        super(NeutronClassifierExtension, self).initialize(connection,
                                                           driver_type)
        print("Sara, About to register RPC consumers.")
        self._register_rpc_consumers(connection)
        print("Sara, initialized Neutron Classifier L2-extension")
    ...
    def handle_notification(self, context, resource_type,
        ...
        """Alerts the l2 extenson agent if a classification or a classification group has been made.
        """
        if event_type == events.CREATED and resource_type==class_group.ClassificationBase.obj_name():
            for class_obj in class_objs:
                self.agent_api.register_classification(class_obj.id,class_obj)

        if event_type == events.CREATED and resource_type==class_group.ClassificationGroup.obj_name():
            for class_obj in class_objs:
                self.agent_api.register_classification_group(class_obj.id,class_obj)
